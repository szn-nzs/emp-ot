...
public:
  myROTSender(std::shared_ptr<Group> _G) : iknp_sender(_G), isSetS(false) {}
  ...
  vector<vector<block>> sendROT(int64_t length) {
    if (!isSetS) {
      error("s not set\n");
    }
  ...
  vector<vector<block>> sendOT(std::span<const block> data0,
                               std::span<const block> data1, int64_t length) {
    if (!isSetS) {
      error("s not set\n");
    }
...
public:
  myROTReceiver(std::shared_ptr<Group> _G) : iknp_receiver(_G), isSetS(false) {}
  ...
  vector<block> recvROT(std::span<const bool> r, int64_t length) {
    if (!isSetS) {
      error("s not set\n");
    }
    if (r.size() != length) {
  ...
  vector<block> recvOT(vector<vector<block>> res, std::span<const bool> r,
                       int64_t length) {
    if (!isSetS) {
      error("s not set\n");
    }
