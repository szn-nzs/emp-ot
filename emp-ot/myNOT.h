...
public:
  myNOTSender(std::shared_ptr<Group> _G, int64_t _maxChoice, int64_t _length)
      : myNOT(_maxChoice, _length), iknp_sender(_G), isSetS(false),
        isSetup(false) {}
  myIKNPSender &getIKNPSender() { return iknp_sender; }
  const vector<block> &getKey0() { return userkey0; }
  const vector<block> &getKey1() { return userkey1; }
  ...
  vector<Point> baseOTMsg1(Point A) { return iknp_sender.baseOTMsg1(A); }
...
public:
  ...
  Point baseOTMsg1() { return iknp_receiver.baseOTMsg1(); }
  ...
  vector<vector<block>> recvPre() {
    ...
    return iknp_receiver.recvPre(std::span(b, maxChoiceBitLength),
                                 maxChoiceBitLength);
  }
  // 上面是iknp的部分，生成ot_instance_num个OT实例，其中recv端持有T

  vector<block> recvROT(vector<vector<block>> res) {
    if (!isSetS) {
      error("s not set\n");
    }
    if (!isSetup) {
    ...
    for (int64_t i = 0; i < maxChoiceBitLength; i += ot_bsize) {
      ...
      // T接下来的ot_size行
      memcpy(pad, T.data() + i,
             min(ot_bsize, maxChoiceBitLength - i) * sizeof(block));
      // 哈希 计算H(t)=H(t)
