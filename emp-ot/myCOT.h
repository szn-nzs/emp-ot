...
class myCOTSender : public myCOT {
public:
    myCOTSender(std::shared_ptr<Group> _G) : iknp_sender(_G), isSetS(false) {}
    ...
    vector<vector<block>> sendCOT(std::span<const block> data0, std::span<const block> data1, int64_t length) {
        if (!isSetS) {
            error("s not set\n");
        }
...
class myCOTReceiver : public myCOT {
public:
    myCOTReceiver(std::shared_ptr<Group> _G) : iknp_receiver(_G), isSetS(false) {}
    ...
    vector<block> recvCOT(vector<vector<block>> res, std::span<const bool> r, int64_t length) {
        if (!isSetS) {
            error("s not set\n");
        }
        if (r.size() != length) {
