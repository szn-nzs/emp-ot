...
double test_mybaseOT(BaseOTSender *otsender, BaseOTReceiver *otreceiver,
                     int64_t length) {
  PRG prg(fix_key);
  PRG prg2;
  ...
  BaseOTSender::EType E =
      otsender->sendE(B, std::span(data0, length), std::span(data1, length));
...
double test_myIKNP(myIKNPSender *sender, myIKNPReceiver *receiver,
                   int64_t length) {
  PRG prg;
...
double test_myROT(myROTSender *sender, myROTReceiver *receiver,
                  int64_t length) {
  PRG prg(fix_key);
  PRG prg2;
  ...
  vector<vector<block>> U =
      receiver->getIKNPReceiver().recvPre(std::span(r, length), length);
  sender->getIKNPSender().sendPre(U, length);

  vector<vector<block>> pad = sender->sendOT(std::span(data0, length),
                                             std::span(data1, length), length);
  vector<block> data = receiver->recvOT(pad, std::span(r, length), length);
  // vector<vector<block>> pad1 = sender->sendROT(length);
  // vector<block> pad2 = receiver->recvROT(std::span(r, length), length);
...
double test_myNOT(myNOTSender *sender, myNOTReceiver *receiver,
                  int64_t maxChoice, int64_t length) {
  ...
  vector<vector<block>> pad = sender->sendROT();
  vector<block> key = receiver->recvROT(pad);
